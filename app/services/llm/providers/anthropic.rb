    ...
    class Anthropic < Base
      ...
      def make_request(messages:, schema:, request_type:)
        ...
        begin
          ...
          request.body = {
            ...
            metadata: {
              user_id: 'dnd-rails-app'  # Adding a user_id for tracking
            }
          }.compact.to_json
      ...
      def parse_successful_response(body)
        ...
        begin
          ...
          # Get the schema for the current request type based on the result structure
          request_type = if result.key?('background')
                          'character_background'
                        elsif result.key?('weapons')
                          'suggest_equipment'
                        elsif result.key?('cantrips')
                          'suggest_spells'
                        else
                          raise Llm::Service::ProviderError, 'Unable to determine request type from response'
                        end
          
          schema = get_schema_for_request(request_type)
          
          # Check required fields
          schema['properties'].each_key do |field|
            if schema['required'].include?(field) && !result[field]
              raise Llm::Service::ProviderError, "Missing required field: #{field}"
            end
          end
          
          # Validate array fields if present
          schema['properties'].each do |field, field_schema|
            next unless field_schema['type'] == 'array' && result[field].is_a?(Array)
            
            min_items = field_schema['minItems']
            max_items = field_schema['maxItems']
            
            if min_items && max_items
              items_count = result[field].length
              unless items_count.between?(min_items, max_items)
                raise Llm::Service::ProviderError, 
                      "#{field} count (#{items_count}) must be between #{min_items} and #{max_items}"
              end
            end
          end
